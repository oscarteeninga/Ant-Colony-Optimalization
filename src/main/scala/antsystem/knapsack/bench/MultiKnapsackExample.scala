package antsystem.knapsack.bench

import antsystem.CriteriaValues
import antsystem.knapsack.multi.Criteria.{Uselessness, Value}
import antsystem.knapsack.multi.{Element, Knapsack}
...
object MultiKnapsackExample {
  val Example: Knapsack = {
    ...
    val items: Set[Element] = criterias.zipWithIndex.zip(weights).collect { case ((criterias, id), weight) if weight > 0 => Element(id, weight, criterias) }.take(20).toSet
