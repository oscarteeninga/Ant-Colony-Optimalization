...
object KnapsackApp extends App with LazyLogging {
  ...
  val best = Knapsack.resolve(25, 100, Knapsack(items, capacity), 0.5, 0.5, 0.5)
  logger.info(s"BEST: ($capacity/${best.capacity}) [${best.amounts} - $best")

  val best2D = Knapsack.resolve2D(1, 100, Knapsack(items, capacity), 0.5, 0.5, 0.5)
  logger.info(s"BEST: ($capacity/${best2D.capacity}) [${best2D.amounts} - $best2D")
}
